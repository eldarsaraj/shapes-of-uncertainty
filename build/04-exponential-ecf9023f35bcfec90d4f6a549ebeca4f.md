...
However, this average does not correspond to a “typical” experience in the way it does under a normal distribution. In fact, most events occur sooner than the average suggests. The mean is pulled outward by relatively rare but long waits (see that the curve is not symmetric). Thus, while the average exists, it does not describe the most common case.
